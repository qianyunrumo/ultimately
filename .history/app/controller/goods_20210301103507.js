...
class GoodsController extends Controller {
  async add() {//新增商品
    let {ctx,app} = this
    let {name,classify,metering,status,price,postage,pic} = ctx.request.body
    let goods = await ctx.model.Goods.findOne({ //查询一个用户信息的数据
      name,classify,metering,status,price,postage,pic
    })
    if (goods) {//如果有
      ctx.body = {
        code: 500,
  ...
  async edit() { //编辑商品
    ...
    let res = await this.ctx.model.User.findByIdAndUpdate({
        _id: id, name, classify, metering, status, price, postage, pic
    })
    if (res) {
      this.ctx.body = {
        code: 200,
