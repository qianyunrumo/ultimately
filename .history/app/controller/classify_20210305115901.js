...
class ClassifyController extends Controller {
  ...
  async addClassify() {//新增商品分类
    ...
    //生成不重复ID
    let random_id =mongoose.Types.ObjextId((Math.random()*10000000).toString(16).substr(0,4)+(new Date()).getTime()+Math.random().toString().substr(2,5))
    ...
    if(flag.length>0){
      let res = await app.model.Classify.findOneAndUpdate({main_class: main_class},{$addToSet:{
