...
class DiscountController extends Controller {
  ...
  async editDiscount() { //编辑商品折扣
    let {id, disName, start_time, end_time,discount_way,dis_status,dis_goods} = this.ctx.request.body
    let res = await this.ctx.model.Discount.findByIdAndUpdate(id, {disName, start_time, end_time, discount_way, dis_status, dis_goods})
    if (res) {
      this.ctx.body = {
        code: 200,
        msg: '修改成功'
